...
(function () {
      ...
      return `
        <div class="pe-group">
          <div class="pe-group-head">
            ...
            <span class="pe-group-select-all" onclick="window._peToggleSamengesteldGroupAll('${escAttr(base)}', ${allChecked ? 'false' : 'true'})">${allChecked ? 'Alles uitvinken' : 'Alles selecteren'}</span>
          </div>
        ...
        <div class="pe-groente-chips">
          ${onderdelen.map(o => {
            ...
            if (o.eenheid === 'gram') {
              ...
              return `
                <label class="pe-groente-chip ${checked?'checked':''}">
                  <input type="checkbox" ${checked?'checked':''} onchange="window._peToggleSamengesteldRow('${key}','${o.id}', this.checked)">
  ...
  window._peToggleSamengesteldRow = function (key, onderdeelId, checked) {
    if (!rowSamengesteldSelectie[key]) rowSamengesteldSelectie[key] = {};
    rowSamengesteldSelectie[key][onderdeelId] = checked;
    const chipInput = document.querySelector(`input[onchange*="_peToggleSamengesteldRow('${key}','${onderdeelId}'"]`);
    if (chipInput) chipInput.closest('.pe-groente-chip')?.classList.toggle('checked', checked);
    const countEl = document.getElementById('pe-cat-selected-count');
    if (countEl) countEl.textContent = `${countChecked()} rij(en) geselecteerd`;
    const addBtn = document.getElementById('pe-cat-add-btn');
