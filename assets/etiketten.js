...
(function () {
  ...
  function buildEtiketData(weekFilter) {
    ...
    // Sorteer op datum (timestamp) dan tijd
  ...
  function renderEtiketten() {
    ...
    container.innerHTML = Object.entries(byDate).sort(([a],[b]) => new Date(a) - new Date(b)).map(([date, rows]) => {
  ...
  (function() {
    ...
    ms.textContent = `
      @media (max-width: 768px) {
        #etiketten-content { padding: 14px !important; }
        .etik-row { flex-direction: column; gap: 8px; }
        .etik-filters { flex-direction: column; }
      }`;
